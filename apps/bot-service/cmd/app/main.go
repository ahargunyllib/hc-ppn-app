...
func main() {
	...
	if env.AppEnv.BotEnabled {
		wg.Add(1)
		go startWhatsAppBot(ctx, psqlDB, &wg)
	}
...
func startWhatsAppBot(ctx context.Context, db *sqlx.DB, wg *sync.WaitGroup) {
	defer wg.Done()

	botService, err := whatsapp.NewWhatsAppBot(ctx, db.DB, db)
	if err != nil {
